...
function renderBadgeNew(newRequestsCount) {
    elements.badgeNew.innerText = newRequestsCount;

    newRequestsCount == 0 ? elements.badgeNew.classList.add('none') : elements.badgeNew.classList.remove('none');
}

function updateFilter(filter) {
    // update select value on page
    elements.select.value = filter.courses
